...
def luhn_algorithm(acc_num):
    digits = digit_to_list(acc_num)

    evens = (sum)(digits[-1::-2])

    odds = (sum)((list)(fmap(lambda x: sum(digit_to_list(x * 2)), digits[-2::-2])))


    return mod(evens + odds, 10) == 0
@_coconut_tco
def digit_to_list(n):
    return _coconut_tail_call((list), map(int, str(n)))

t1 = 79927398713  # True
t2 = 79927398714  # False

(print)(luhn_algorithm(t1))
(print)(luhn_algorithm(t2))
